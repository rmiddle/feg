...
<table cellpadding="1" cellspacing="0" border="0" width="100%" class="tableRowBg">
	...
	{/if}
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			<td><a href="javascript:;" onclick="genericAjaxPanel('c=setup&a=showWorkerPeek&id={$result.w_id}&view_id={$view->id|escape:'url'}',this,false,'550px');" style="{if $result.w_is_disabled}color:rgb(120,0,0);font-style:italic;{/if}">{$result.$column}</a>&nbsp;</td>
	...
	<tr>
		<td colspan="2">
			{if $active_worker && $active_worker->is_superuser}
				<button type="button" onclick="genericAjaxPanel('c=setup&a=showWorkersBulkPanel&view_id={$view->id}&ids=' + Devblocks.getFormEnabledCheckboxValues('viewForm{$view->id}','row_id[]'),this,false,'500px');"><img src="{devblocks_url}c=resource&p=usermeet.core&f=images/folder_gear.gif{/devblocks_url}" align="top"> bulk update</button>
			{/if}
		</td>
	</tr>
	{/if}
