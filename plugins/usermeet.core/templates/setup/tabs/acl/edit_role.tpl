		...
		<td width="100%" valign="top" colspan="2">
			{foreach from=$plugins item=plugin key=plugin_id}
				{if $plugin->enabled}
					...
					<label>
					{if $plugin->id=="usermeet.core"}
						<input type="hidden" name="acl_privs[]" value="plugin.usermeet.core">
					{else}
					...
					<b>{$plugin->name}</b></label><br>
						<div id="privs{$plugin_id}" style="padding-left:10px;margin-bottom:5px;display:{if $plugin->id=="usermeet.core" || isset($role_privs.$plugin_priv)}block{else}none{/if}">
	...
	<tr>
		<td colspan="2">
			{if isset($license.serial) && !isset($license.a)}
				...
				{if $active_worker->is_superuser}<button type="button" onclick="if(confirm('Are you sure you want to delete this role?')){literal}{{/literal}this.form.do_delete.value='1';this.form.submit();{literal}}{/literal}"><img src="{devblocks_url}c=resource&p=usermeet.core&f=images/delete2.gif{/devblocks_url}" align="top"> {$translate->_('common.delete')|capitalize}</button>{/if}
			{/if}
		</td>
	</tr>
</table>
