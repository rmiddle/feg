...
class UmTranslatorsSetupTab extends Extension_SetupTab {
	...
	function saveTab() {
		...
		DevblocksPlatform::redirect(new DevblocksHttpResponse(array('setup','translators')));
		exit;
	}
...
class Um_TranslationView extends Um_AbstractView {
	...
	function render() {
		...
		$tpl->display('file:' . DEVBLOCKS_PLUGIN_PATH . 'usermeet.translators/templates/setup/translations/view.tpl');
	}
	...
	function renderCriteria($field) {
		...
		switch($field) {
			...
			case SearchFields_Translation::LANG_CODE:
				...
				$tpl->assign('langs', $langs);
				$tpl->display('file:' . DEVBLOCKS_PLUGIN_PATH . 'usermeet.core/templates/internal/views/criteria/__language.tpl');
				break;
			default:
				echo '';
				break;
