...
{*		<button type="button" onclick="genericPanel.dialog('close');$('#message_reciptient_peek_{$id}_status').load('{devblocks_url}ajax.php?c=customer&a=handleTabAction&tab=feg.customer.tab.recipient&action=setMessageRecipientStatus&id={$message_recipient_id}&status=6&view_id={$view->id|escape:'url'}{/devblocks_url}');"><img src="{devblocks_url}c=resource&p=feg.core&f=images/delete2.gif{/devblocks_url}" align="top"> {$translate->_('feg.message_recipient.submit.permfail')}</button>
*}
{/if}
