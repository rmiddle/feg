...
{if $message->import_status == 2}
	{if $active_worker->hasPriv('core.access.message.reprocess')}
		<a href="javascript:;" onclick="$('#span_message_import_status').load('{devblocks_url}ajax.php?c=customer&a=handleTabAction&tab=feg.customer.tab.standard.messages&action=setMessageStatus&id={$message->id}&status=0{/devblocks_url}');">
