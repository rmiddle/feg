...
{/if}
	...
	<tr id='tr_address' {if $customer_recipient->type == '255'}style="display:none"{/if}>
		...
		<td id='tr_address_input' width="100%">
			<input type="text" name="recipient_address" id="recipient_address" value="{if $customer_recipient->type != '255'}{$customer_recipient->address|escape}{/if}" style="width:98%;">
		</td>
	</tr>
		...
		$('#recipient_type').change(function() {
			...
			switch (sel)
			{
				...
				case "255": 	{*SLAVE*}
					...
					{if isset($customer_account_master)}
						$("#tr_address_current_account_number").show();  // Only orignal setting was type slave.
					{/if}
					break
