...
<input type="hidden" name="a" value="doImportBulkUpdate">
...
<table cellspacing="0" cellpadding="2" width="100%">
	<tr>
		...
		<td width="100%">
			<select name="Import_is_disabled">
				<option value="">&nbsp;</option>
				<option value="0">{$translate->_('common.enable')}</option>
				<option value="1">{$translate->_('common.disable')}</option>
			</select>
			
			<button type="button" onclick="this.form.Import_is_disabled.selectedIndex=1;">{'common.enable'|devblocks_translate|capitalize}</button>
			<button type="button" onclick="this.form.Import_is_disabled.selectedIndex=2;">{'common.disable'|devblocks_translate|capitalize}</button>
		</td>
	</tr>
</table>
