	...
	{/if}
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column=="w_email"}
				<td><a href="javascript:;" onclick="genericAjaxPanel('c=contacts&a=showAddressPeek&email={$result.w_email|escape:'url'}&view_id={$view->id}',null,false,'500',ajax.cbAddressPeek);" title="{$result.w_email|escape}">{$result.w_email|truncate:64:'...':true:true}</a></td>
