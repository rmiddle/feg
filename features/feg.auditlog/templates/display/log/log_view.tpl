			...
			{elseif $column=="l_message_recipient_id"}
				<td>
					{if $result.l_message_recipient_id==0}
						{$translate->_('customer.display.auto')|capitalize}
					{else}
						<a href="javascript:;" onclick="genericAjaxPanel('c=customer&a=handleTabAction&tab=feg.customer.tab.recent.messages&action=showMessagePeek&id={l_message_recipient_id}&customer_id={$result.l_account_id}&view_id={$view->id|escape:'url'}',null,false,'550');">{l_message_recipient_id}&nbsp;</a>
					{/if}				
				</td>
