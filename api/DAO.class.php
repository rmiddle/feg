...
class DAO_Recipient extends DevblocksORMHelper {
	const ID = 'id';
	const IS_DISABLED = 'is_disabled';
	...
	static function create($fields) {
		...
		$sql = sprintf("INSERT INTO recipient (id) ".
			"VALUES (%d)",
			$id
		);
		$db->Execute($sql);
	...
	static function update($ids, $fields) {
		parent::_update($ids, 'recipient', $fields);
	}
	
	static function updateWhere($fields, $where) {
		parent::_updateWhere('recipient', $fields, $where);
	}
	
	/**
	 * @param string $where
	 * @return Model_Recipient[]
	 */
	static function getWhere($where=null) {
		$db = DevblocksPlatform::getDatabaseService();
		
		$sql = "SELECT id, is_disabled, recipient_name, type, address, notes ".
			"FROM recipient ".
			(!empty($where) ? sprintf("WHERE %s ",$where) : "").
			"ORDER BY id asc";
		$rs = $db->Execute($sql);
	...
	static function delete($ids) {
		...
		$db->Execute(sprintf("DELETE FROM recipient WHERE id IN (%s)", $ids_list));
    ...
    static function search($columns, $params, $limit=10, $page=0, $sortBy=null, $sortAsc=null, $withCounts=true) {
		$db = DevblocksPlatform::getDatabaseService();
		$fields = SearchFields_Recipient::getFields();
		...
		$select_sql = sprintf("SELECT ".
			"recipient.id as %s, ".
			"recipient.is_disabled as %s, ".
			"recipient.recipient_name as %s, ".
			"recipient.type as %s, ".
			"recipient.address as %s, ".
			"recipient.notes as %s ",
		...
		$sql = 
			...
			$join_sql.
			$where_sql.
			($has_multiple_values ? 'GROUP BY recipient.id ' : '').
			$sort_sql;
		...
		while($row = mysql_fetch_assoc($rs)) {
			...
			$object_id = intval($row[SearchFields_Recipient::ID]);
			$results[$object_id] = $result;
		}
		...
		if($withCounts) {
			$count_sql = 
				($has_multiple_values ? "SELECT COUNT(DISTINCT recipient.id) " : "SELECT COUNT(recipient.id) ").
				$join_sql.
				$where_sql;
			$total = $db->GetOne($count_sql);
		}
...
class SearchFields_Recipient implements IDevblocksSearchFields {
	const ID = 'r_id';
	const IS_DISABLED = 'r_is_disabled';
	const RECIPIENT_NAME = 'r_recipient_name';
	const TYPE = 'r_type';
	const ADDRESS = 'r_address';
	const NOTES = 'r_notes';
	...
	static function getFields() {
		...
		$columns = array(
			self::ID => new DevblocksSearchField(self::ID, 'recipient', 'id', $translate->_('id')),
			self::IS_DISABLED => new DevblocksSearchField(self::IS_DISABLED, 'recipient', 'is_disabled', $translate->_('is_disabled')),
			self::RECIPIENT_NAME => new DevblocksSearchField(self::RECIPIENT_NAME, 'recipient', 'recipient_name', $translate->_('recipient_name')),
			self::TYPE => new DevblocksSearchField(self::TYPE, 'recipient', 'type', $translate->_('type')),
			self::ADDRESS => new DevblocksSearchField(self::ADDRESS, 'recipient', 'address', $translate->_('address')),
			self::NOTES => new DevblocksSearchField(self::NOTES, 'recipient', 'notes', $translate->_('notes')),
		);
